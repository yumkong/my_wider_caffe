...
%% -------------------- INIT_MODEL --------------------
% 0709: vgg16 model cannot use because device memory is not enough
%model_dir                   = fullfile('..', 'output', 'faster_rcnn_final', 'faster_rcnn_VOC0712_vgg_16layers'); %% VGG-16
% ZF model downloaded from official website
%model_dir                   = fullfile('..', 'output', 'faster_rcnn_final', 'faster_rcnn_VOC0712_ZF'); %% ZF
% ZF model trained by myself
model_dir  = fullfile( 'output', 'faster_rcnn_final', 'faster_rcnn_VOC2007_ZF'); %% ZF
...
function proposal_detection_model = load_proposal_detection_model(model_dir)
    ...
    %0727 added to test not shared network
    proposal_detection_model.is_share_feature = 1;
    if ~proposal_detection_model.is_share_feature
        proposal_detection_model.detection_net_def ...
                                = fullfile(model_dir, 'detection_test_spare.prototxt');
    end
end
