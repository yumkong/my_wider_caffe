...
%0414 always use figure 2 to plot
figure(2), clf ; subplot(np,1,1);
errorbar(1:n, fmu, fmi, fmx, 'mo') ;
grid on ;
xlabel('layer') ;
ylabel('filters') ;
...
if nargin > 1
  subplot(np,1,3) ;
  errorbar(1:n, xmu, xmi, xmx, 'mo') ;
  grid on ;
  xlabel('layer') ;
  ylabel('x') ;

  subplot(np,1,4) ;
  errorbar(1:n, dxmu, dxmi, dxmx, 'mo') ;
  grid on ;
  xlabel('layer') ;
  ylabel('dzdx') ;

  subplot(np,1,5) ;
  errorbar(1:n, dfmu, dfmi, dfmx, 'mo') ;
  grid on ;
  xlabel('layer') ;
  ylabel('dfilters') ;

  subplot(np,1,6) ;
  errorbar(1:n, dbmu, dbmi, dbmx, 'mo') ;
  grid on ;
  xlabel('layer') ;
  ylabel('dbiases') ;
end
